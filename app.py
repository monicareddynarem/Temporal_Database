...
def run_live_benchmark(symbol, start_time, end_time):
    ...
    # Standard DB Query - FIXED FOR ARRAY UNNESTING
    raw_q = """
        SELECT sum(v), count(v) 
        FROM raw_ticks_bucketed, unnest(volumes) as v 
    ...
    metrics.append({
        "Architecture": "Standard DB (Bucketed Arrays)",
...
def fetch_storage_stats():
    """Correctly calculates size of partitioned tables - FIXED TABLE NAMES"""
    conn = get_db_connection()
    try:
        query = """
            SELECT 
            ...
            WHERE parent.relname IN ('raw_ticks_bucketed', 'ohlcv_1m')
...
with st.sidebar:
    st.title("Controls")
    # FIXED: Swapped to NSE tickers to match the Python data generator!
    selected_symbol = st.selectbox("Symbol", ['RELIANCE', 'TCS', 'INFY', 'HDFCBANK', 'ICICIBANK', 'SBIN', 'BHARTIARTL', 'ITC', 'LT', 'BAJFINANCE'])
...
with tab_market:
    ...
    if df.empty:
        ...
        latest_price = df.iloc[-1]['close_price'] # Fixed: Grab the last chronological price, not the first
        ...
        c1.metric("Latest Price", f"₹{latest_price:.2f}") # Fixed: Swapped to Rupees
...
with tab_bench:
    ...
    if st.button("Run Live Analytics Benchmark", type="primary"):
        ...
        # FIXED: Updated key to match the new architecture name
        raw_l = bench_df.loc["Standard DB (Bucketed Arrays)", "Latency (ms)"]
        opt_l = bench_df.loc["Optimized (Aggregates)", "Latency (ms)"]
        if opt_l > 0:
            st.success(f"**Optimization Result:** Aggregate queries are **{raw_l/opt_l:.1f}x faster** than unrolling raw arrays.")
    ...
    if not stats.empty:
        ...
        try:
            # FIXED: Looking for the bucketed table
            raw_b = stats[stats['Table'] == 'raw_ticks_bucketed']['Bytes'].values[0]
            ...
            if raw_b > 0:
             ...
             st.warning("Not enough data to calculate compression ratio yet.")